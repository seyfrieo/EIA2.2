...
namespace aquarium2 {
    ...
    function init(): void {
        ...
        for (let i: number = 0; i < 15; i++)
            movingObjects.push(new Bubble());

        //fisch1
        movingObjects.push(new Fisch(300, 200, 1));
        //fisch2
        movingObjects.push(new Fisch(500, 300, 2));
    ...
    function createFlocken(_e: MouseEvent): void {
        let x: number = _e.clientX;
        let y: number = _e.clientY;

        for (let i: number = 0; i < 5; i++) {
            movingObjects.push(new Flocke(x, y));
        }
    }
